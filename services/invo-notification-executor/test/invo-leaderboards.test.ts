...
test('canonical UI leaderboard horizons normalize but stay explicit', () => {
...
test('leaderboard item persists exact horizon and rank provenance', () => {
...
test('same portfolio on multiple horizons is never overwritten across leaderboards', () => {
  ...
  const shared = report.crossHorizon.find(row => row.portfolioId === 'p-shared');
  assert.ok(shared);
  assert.equal(shared.appearanceCount, 3);
  assert.deepEqual(shared.ranks, { '1D': 1, '1W': 2, AT: 1 });
  assert.deepEqual(shared.returns, { '1D': 12, '1W': 33, AT: 300 });

  const immutable = readFileSync(snapshots, 'utf8').trim().split('\n').map(line => JSON.parse(line));
  assert.equal(immutable.length, 5);
  assert.deepEqual(immutable.map(row => [row.horizon, row.rank, row.portfolioId]), [
    ['1D', 1, 'p-shared'],
    ['1D', 2, 'p-day'],
    ['1W', 1, 'p-week'],
    ['1W', 2, 'p-shared'],
...
test('pagination preserves global rank within each horizon', () => {
  ...
  ledger.observePage([portfolio('p51', 'o51', 1), portfolio('p52', 'o52', 2)], '1M', now, 50);
  const month = ledger.report().latestByHorizon['1M'];
  assert.deepEqual(month.map(row => row.rank), [51, 52]);
});

test('a new observation replaces current horizon view without deleting immutable history', () => {
  const dir = mkdtempSync(join(tmpdir(), 'invo-leaderboards-refresh-'));
  const state = join(dir, 'state.json');
  const snapshots = join(dir, 'snapshots.jsonl');
  const ledger = new InvoLeaderboardLedger(state, snapshots);
  ledger.observePage([portfolio('old', 'o1', 10)], '1Y', now, 0);
  ledger.observePage([portfolio('new', 'o2', 20)], '1Y', now + 60_000, 0);
  assert.deepEqual(ledger.report().latestByHorizon['1Y'].map(row => row.portfolioId), ['new']);
