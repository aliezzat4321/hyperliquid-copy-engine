...
PHASE_EVIDENCE_SCHEMAS = {
    "PRODUCTION_VALIDATION": ("runtime-observer", "runtime_healthy",
                              ("exact_code_running", "health_checks_pass")),
    "DEPLOY": ("deployment-controller", "deployment_matches",
               ("install_succeeded", "exact_code_installed")),
    "MEASUREMENT": ("measurement-runner", "measurement_passes",
                    ("manifest_hashes_match", "policy_thresholds_pass")),
    "POST_MERGE_EVIDENCE": ("prospective-runner", "prospective_window_passes",
                            ("window_is_prospective", "manifest_hashes_match")),
    "EVIDENCE_AUDIT": ("evidence-auditor", "audit_passes",
                       ("inputs_reconciled", "economics_recomputed")),
    "FINAL_VERDICT": ("promotion-policy-verifier", "policy_passes",
                      ("policy_version_matches", "all_policy_gates_pass")),
    "PRODUCTION_AUDIT": ("storage-auditor", "storage_audit_passes",
                         ("inventory_complete", "hashes_verified")),
    "IMMUTABLE_PLAN": ("storage-plan-verifier", "plan_is_immutable",
                       ("plan_hash_verified", "scope_is_bounded")),
    "DESTRUCTIVE_REVIEW": ("opus-review-verifier", "review_passes",
                           ("exact_plan_reviewed", "review_verdict_pass")),
    "AUTHORIZED_APPLY": ("authorization-controller", "apply_is_authorized",
                         ("authorization_valid", "scope_matches_plan")),
    "STABILITY_VALIDATION": ("storage-auditor", "stability_window_passes",
                             ("window_complete", "stability_thresholds_pass")),
}
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "review_profiles": {
        ...
        "QUANT": ["OPUS_DESIGN", "FROZEN_CONTRACT", "DETERMINISTIC_PREFLIGHT",
                  "CODEX_REVIEW", "SONNET_CHALLENGE", "PROSPECTIVE_EVIDENCE", "OPUS_FINAL"],
        "DESTRUCTIVE": ["DETERMINISTIC_PREFLIGHT", "CODEX_REVIEW", "OPUS_FINAL"],
    },
    "initial_routes": {
...
class Ledger:
    def __init__(self, path: Path, evidence_root: Path | None = None) -> None:
        ...
        for name in ("limit_text", "systemd_unit", "review_profile", "claim_slot",
                     "claim_token", "lease_until", "lifecycle_phase",
    ...
    def record_acceptance_evidence(self, *, issue_number: int, requirement: str,
        ...
        """Verify a trusted immutable artifact and persist the recomputed verdict."""
        artifact, artifact_path, artifact_hash, predicate = self._verify_artifact(
            issue_number, requirement, phase, evidence, expected_merged_sha
        )
        identity = canonical_json({"issue": issue_number, "requirement": requirement,
                                   "phase": phase, "artifact_hash": artifact_hash})
        ...
        self.db.execute(
            ...
            (evidence_id, issue_number, requirement, phase, str(artifact["producer"]),
             str(artifact["observed_at"]), artifact["window"]["start"],
             artifact["window"]["end"], artifact["code_sha"], artifact_hash,
             canonical_json({"artifact_path": str(artifact_path),
                             "schema_version": ACCEPTANCE_ARTIFACT_SCHEMA,
                             "policy_version": ACCEPTANCE_POLICY_VERSION}),
             canonical_json(artifact["result"]), int(predicate), utcnow()),
        )
        self.db.commit()
        return evidence_id

    def _verify_artifact(self, issue_number: int, requirement: str, phase: str,
                         evidence: dict[str, Any], expected_merged_sha: str | None = None
                         ) -> tuple[dict[str, Any], Path, str, bool]:
        if phase not in PHASE_EVIDENCE_SCHEMAS or phase not in COMPLETION_REQUIREMENTS.get(
            requirement, ()
        ):
            raise ValueError("INCOMPATIBLE_ACCEPTANCE_PHASE")
        required = ("source", "artifact_path", "artifact_hash")
        if any(not evidence.get(key) for key in required):
            raise ValueError("INCOMPLETE_ACCEPTANCE_EVIDENCE")
        producer, predicate_name, required_checks = PHASE_EVIDENCE_SCHEMAS[phase]
        if evidence["source"] != producer:
            raise ValueError("UNTRUSTED_EVIDENCE_SOURCE")
        path = Path(str(evidence["artifact_path"])).resolve()
        try:
            path.relative_to(self.evidence_root / producer)
        except ValueError as exc:
            raise ValueError("UNTRUSTED_EVIDENCE_PATH") from exc
        try:
            raw = path.read_bytes()
        except OSError as exc:
            raise ValueError("UNVERIFIABLE_EVIDENCE_ARTIFACT") from exc
        actual_hash = hashlib.sha256(raw).hexdigest()
        if not re.fullmatch(r"[0-9a-f]{64}", str(evidence["artifact_hash"])) \
                or evidence["artifact_hash"] != actual_hash:
            raise ValueError("EVIDENCE_ARTIFACT_HASH_MISMATCH")
        try:
            artifact = json.loads(raw)
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise ValueError("INVALID_EVIDENCE_ARTIFACT") from exc
        if raw != canonical_json(artifact).encode():
            raise ValueError("NONCANONICAL_EVIDENCE_ARTIFACT")
        expected = {
            "schema_version": ACCEPTANCE_ARTIFACT_SCHEMA,
            "policy_version": ACCEPTANCE_POLICY_VERSION,
        ...
        artifact_keys = {*expected, "code_sha", "observed_at", "window", "artifacts",
                         "result"}
        if (not isinstance(artifact, dict) or set(artifact) != artifact_keys
                or any(artifact.get(k) != v for k, v in expected.items())):
            raise ValueError("INCOMPATIBLE_EVIDENCE_ARTIFACT")
        if not expected_merged_sha or not re.fullmatch(r"[0-9a-f]{40}", expected_merged_sha):
            raise ValueError("MISSING_EXACT_MERGED_SHA")
        if artifact.get("code_sha") != expected_merged_sha:
            raise ValueError("EVIDENCE_CODE_SHA_MISMATCH")
        observed = parse_utc(str(artifact.get("observed_at") or ""))
        window = artifact.get("window")
        start = parse_utc(str(window.get("start") or "")) if isinstance(window, dict) else None
        end = parse_utc(str(window.get("end") or "")) if isinstance(window, dict) else None
        now = dt.datetime.now(dt.timezone.utc)
        if not observed or not start or not end or not start <= end <= observed <= now:
            raise ValueError("INVALID_EVIDENCE_WINDOW")
        if now - observed > ACCEPTANCE_MAX_AGE:
            raise ValueError("STALE_ACCEPTANCE_EVIDENCE")
        artifacts = artifact.get("artifacts")
        if not isinstance(artifacts, list) or not artifacts:
            raise ValueError("INVALID_IMMUTABLE_ARTIFACT_HASHES")
        for item in artifacts:
            if not isinstance(item, dict) or set(item) != {"path", "sha256"}:
                raise ValueError("INVALID_IMMUTABLE_ARTIFACT_HASHES")
            input_path = Path(str(item["path"])).resolve()
            try:
                input_path.relative_to(self.evidence_root / producer)
                input_bytes = input_path.read_bytes()
            except (ValueError, OSError) as exc:
                raise ValueError("UNVERIFIABLE_REFERENCED_ARTIFACT") from exc
            if (not re.fullmatch(r"[0-9a-f]{64}", str(item["sha256"]))
                    or hashlib.sha256(input_bytes).hexdigest() != item["sha256"]):
                raise ValueError("REFERENCED_ARTIFACT_HASH_MISMATCH")
        result = artifact.get("result")
        if not isinstance(result, dict) or set(result) != set(required_checks) \
                or any(result[check] not in (True, False) for check in required_checks):
            raise ValueError("INVALID_PHASE_PREDICATE_INPUT")
        # The stored/caller predicate is never consulted: each phase predicate is the
        # verifier-owned conjunction of its canonical, phase-specific check set.
        return artifact, path, actual_hash, all(result[check] is True for check in required_checks)

    def _row_is_verified(self, row: sqlite3.Row) -> bool:
        try:
            manifests = json.loads(row["manifests_json"])
            task = self.db.execute(
                "SELECT target_sha FROM tasks WHERE issue_number=? AND lifecycle_phase=? "
                "ORDER BY created_at DESC LIMIT 1", (row["issue_number"], row["phase"])
            ).fetchone()
            evidence = {"source": row["source"], "artifact_path": manifests["artifact_path"],
                        "artifact_hash": row["data_hash"]}
            _, _, _, predicate = self._verify_artifact(
                int(row["issue_number"]), str(row["requirement"]), str(row["phase"]),
                evidence, str(task["target_sha"]) if task else None,
            )
            return predicate
    ...
    def proven_requirements(self, issue_number: int) -> set[str]:
        rows = self.db.execute(
            "SELECT * FROM acceptance_evidence WHERE issue_number=?", (issue_number,)
        ).fetchall()
        observed: dict[str, set[str]] = {}
        for row in rows:
            if self._row_is_verified(row):
                observed.setdefault(str(row["requirement"]), set()).add(str(row["phase"]))
    ...
    def phase_is_proven(self, issue_number: int, requirement: str, phase: str) -> bool:
        rows = self.db.execute(
            "SELECT * FROM acceptance_evidence WHERE issue_number=? AND requirement=? "
            "AND phase=?", (issue_number, requirement, phase)
        ).fetchall()
        return any(self._row_is_verified(row) for row in rows)
...
def parse_review_profile(body: str, cfg: dict[str, Any], task_class: str) -> str:
    """Resolve a profile without permitting metadata to weaken its risk floor."""
    values = _machine_values(body, "AI_TEAM_REVIEW_PROFILE")
    if len(values) > 1:
        raise ValueError("INVALID_REVIEW_PROFILE: duplicate explicit profile")
    required = required_review_profile(
        task_class,
        protected=acceptance_flag(body, "AI_TEAM_PROTECTED_CHANGE"),
        live_sensitive=bool(re.search(r"(?mi)^\s*LIVE-SENSITIVE\s*:\s*YES\s*$", body or "")),
    )
    profile = values[0] if values else required
    if profile not in cfg.get("review_profiles", {}):
        raise ValueError(f"INVALID_REVIEW_PROFILE: {profile}")
    if not review_profile_meets_floor(profile, required):
        raise ValueError(
            f"REVIEW_PROFILE_BELOW_REQUIRED_FLOOR: requested={profile} required={required}"
        )
    return profile


def required_review_profile(task_class: str, *, protected: bool = False,
                            live_sensitive: bool = False) -> str:
    """Return the monotonic minimum imposed by class and sensitivity metadata."""
    aliases = {
    ...
    required = aliases.get(task_class, task_class)
    if required not in {"ROUTINE", "ENGINE_CRITICAL", "QUANT", "DESTRUCTIVE"}:
        required = "ROUTINE"
    if (protected or live_sensitive) and required == "ROUTINE":
        return "ENGINE_CRITICAL"
    return required


def review_profile_meets_floor(requested: str, required: str) -> bool:
    """Compare named profiles using the reviewed partial order, not list length."""
    allowed_upgrades = {
        "ROUTINE": {"ROUTINE", "ENGINE_CRITICAL", "QUANT", "DESTRUCTIVE"},
        "ENGINE_CRITICAL": {"ENGINE_CRITICAL", "QUANT"},
        "QUANT": {"QUANT"},
        "DESTRUCTIVE": {"DESTRUCTIVE", "QUANT"},
    }
    return requested in allowed_upgrades.get(required, set())
...
def task_review_profile(cfg: dict[str, Any], task: sqlite3.Row) -> list[str]:
    ...
    if explicit:
        ...
        required = required_review_profile(str(task["task_class"]))
        if not review_profile_meets_floor(str(explicit), required):
            raise RuntimeError(
                "REVIEW_PROFILE_BELOW_REQUIRED_FLOOR: "
                f"requested={explicit} required={required}"
            )
...
class Orchestrator:
    ...
    def complete_acceptance_phase(self, task: sqlite3.Row,
        ...
        self.ledger.record_acceptance_evidence(
            ...
            phase=phase, evidence=evidence, expected_merged_sha=str(task["target_sha"]),
        )
        if not predicate:
    ...
    def reconcile_closed_issue_tasks(self) -> int:
        """Retire active work whose canonical GitHub target is already closed.

        This runs before dispatch so a closed Issue or unmerged PR cannot leave a
        RETRY/wait task projected as current runtime state.  Merged PRs remain valid
        targets for post-merge acceptance work.  GitHub lookup failures are fail-safe:
        defer that target and continue reconciling and dispatching unrelated work.
        """
        placeholders = ",".join("?" for _ in ACTIVE_STATUSES)
        ...
        pr_states: dict[int, tuple[str, bool] | None] = {}
        retired = 0
        for task in rows:
            ...
            obsolete_reason = "OBSOLETE_CLOSED_ISSUE" if issue_states[number] == "closed" else None
            pr_number = int(task["pr_number"]) if task["pr_number"] is not None else None
            if obsolete_reason is None and pr_number is not None:
                if pr_number not in pr_states:
                    try:
                        pr = self.gh.pr(pr_number)
                        pr_states[pr_number] = (
                            str(pr.get("state") or "open").lower(),
                            bool(pr.get("merged") or pr.get("merged_at")),
                        )
                    except Exception as exc:
                        pr_states[pr_number] = None
                        self.runtime.event(
                            "CLOSED_PR_RECONCILIATION_DEFERRED",
                            issue=number, pr=pr_number, error=str(exc)[:500],
                        )
                pr_state = pr_states[pr_number]
                if pr_state is not None and pr_state[0] == "closed" and not pr_state[1]:
                    obsolete_reason = "OBSOLETE_CLOSED_PR"
            if obsolete_reason is None:
                continue
            self.reap_stale_child(task)
            self.ledger.update(
                task["id"], status="DONE", retry_at=None, systemd_unit=None,
                last_error=obsolete_reason, failure_class=None,
                lifecycle_phase="OBSOLETE",
                next_action=(
                    "retired because canonical GitHub issue is closed"
                    if obsolete_reason == "OBSOLETE_CLOSED_ISSUE"
                    else "retired because target pull request is closed without merge"
                ),
            )
            self.runtime.event(
                ("OBSOLETE_CLOSED_TASK_RETIRED"
                 if obsolete_reason == "OBSOLETE_CLOSED_ISSUE"
                 else "OBSOLETE_CLOSED_PR_TASK_RETIRED"),
                assignment_id=task["id"], issue=number, pr=task["pr_number"],
                status="OBSOLETE",
                unrelated_work_continuing=True,
            )
            retired += 1
    ...
    def cycle(self) -> None:
        ...
        # Admit one READY/queued issue each cycle before leasing task-local slots.
...
"""
    ...
    def enqueue_review(self, parent: sqlite3.Row, pr_number: int, sha: str) -> None:
        ...
        explicit_profile = parse_review_profile(body, self.cfg, str(parent["task_class"]))
...
"""
    ...
    def handle_ci(self, task: sqlite3.Row) -> None:
        ...
        if not re.fullmatch(r"[0-9a-f]{40}", merged_sha):
            self.block(task, "merge response omitted exact merged commit SHA")
            return
        contract = self.completion_contract(issue)
        ...
        if contract["close_on_merge"]:
            self.ledger.update(task["id"], lifecycle_phase="PROVEN")
            self.runtime.event("ACCEPTANCE_PROVEN", assignment_id=task["id"],
                               issue=task["issue_number"], target_sha=merged_sha,
                               status="PROVEN", lifecycle_phase="PROVEN")
            self.finalize_proven_issue(issue, self.ledger.get(str(task["id"])),
