...
class Ledger:
    def __init__(self, path: Path) -> None:
        ...
        for name in ("limit_text", "systemd_unit", "review_profile"):
    ...
    def has_queue_claim_conflict(self) -> bool:
        ...
        row = self.db.execute(
            ...
            "OR task_type='PROSPECTIVE_EVIDENCE') "
...
def parse_review_profile(body: str, cfg: dict[str, Any], task_class: str) -> str:
    """Return the authoritative explicit profile, failing closed on bad metadata."""
    ...
    aliases = {
        ...
        "MAJOR_ARCHITECTURE": "ENGINE_CRITICAL",
        "UNRESOLVED_DISAGREEMENT": "QUANT",
    }
    profile = values[0] if values else aliases.get(task_class, task_class)
...
def deterministic_preflight(workdir: Path, sha: str, changed: list[str]) -> list[dict[str, Any]]:
    """Run repository-wide deterministic lint and the complete relevant test set."""
    commands: list[list[str]] = [
        [sys.executable, "-m", "ruff", "check", "."],
        [sys.executable, "-m", "ruff", "format", "--check", "."],
    ]
    ai_tests = sorted(str(path.relative_to(workdir))
                      for path in (workdir / "tests").glob("test_ai_team*.py"))
    commands.append([sys.executable, "-m", "pytest", "-q", *ai_tests])
    blockers = []
