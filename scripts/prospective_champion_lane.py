...
def main() -> None:
    ...
    for target_spec in targets:
                    ...
                    target["scenarios"].append(
                        {
                            ...
                            "executable_events": int(summary["executable_events"]),
                            "missed_events": int(summary["missed_events"]),
        ...
        target["queue_coin_l2_replay"] = {
            "coin": target_spec["coin"],
            "scenario_rows": len(target["scenarios"]),
            "executable_events": sum(
                int(row["executable_events"]) for row in target["scenarios"]
            ),
            "missed_events": sum(int(row["missed_events"]) for row in target["scenarios"]),
            "status": "OBSERVED" if target["scenarios"] else "NOT_EVALUATED",
        }
        if not primary:
