...
def test_window_ids_include_wallet_and_coin_candidate_identity() -> None:
    events = tuple(_event(index * 1_000_000_000, index) for index in range(1, 11))
    base = _window_id(WALLET, "HYPE", events, 6, 123)
    assert _window_id("0x" + "b" * 40, "HYPE", events, 6, 123) != base
    assert _window_id(WALLET, "BTC", events, 6, 123) != base
