...
def test_routine_may_explicitly_upgrade_and_invalid_profile_fails_closed():
...
@pytest.mark.parametrize("requested", ["ROUTINE", "ENGINE_CRITICAL"])
def test_quant_classes_reject_profiles_below_required_floor(task_class, requested):
    with pytest.raises(ValueError, match="REVIEW_PROFILE_BELOW_FLOOR"):
        orch.parse_review_profile(
            f"AI_TEAM_REVIEW_PROFILE={requested}", orch.DEFAULT_CONFIG, task_class
        )


@pytest.mark.parametrize("task_class", ["MAJOR_ARCHITECTURE", "DESTRUCTIVE"])
def test_destructive_classes_reject_routine_profile(task_class):
    with pytest.raises(ValueError, match="REVIEW_PROFILE_BELOW_FLOOR"):
        orch.parse_review_profile(
            "AI_TEAM_REVIEW_PROFILE=ROUTINE", orch.DEFAULT_CONFIG, task_class
        )


def test_protected_and_sensitive_paths_cannot_be_downgraded():
    with pytest.raises(ValueError, match="required ENGINE_CRITICAL"):
        orch.parse_review_profile(
            "AI_TEAM_PROTECTED_CHANGE=YES\nAI_TEAM_REVIEW_PROFILE=ROUTINE",
            orch.DEFAULT_CONFIG,
            "ROUTINE",
        )
    with pytest.raises(ValueError, match="required DESTRUCTIVE"):
        orch.parse_review_profile(
            "AI_TEAM_REVIEW_PROFILE=ENGINE_CRITICAL",
            orch.DEFAULT_CONFIG,
            "ROUTINE",
            ["src/hlcopy/trading/permissions.py"],
        )


def test_control_plane_path_requires_engine_critical_without_issue_flag():
    with pytest.raises(ValueError, match="required ENGINE_CRITICAL"):
        orch.parse_review_profile(
            "AI_TEAM_REVIEW_PROFILE=ROUTINE",
            orch.DEFAULT_CONFIG,
...
@pytest.mark.parametrize("path", [
    "services/invo-notification-executor/src/env.ts",
    "scripts/deploy_ai_team.sh",
    "scripts/storage_controller.py",
    "config/market_tape_lifecycle.json",
])
def test_live_and_destructive_storage_paths_require_destructive_profile(path):
    with pytest.raises(ValueError, match="required DESTRUCTIVE"):
        orch.parse_review_profile(
            "AI_TEAM_REVIEW_PROFILE=ENGINE_CRITICAL",
            orch.DEFAULT_CONFIG,
...
def test_configured_profile_lattice_itself_fails_closed_if_weakened():
    cfg = {**orch.DEFAULT_CONFIG, "review_profiles": {
        **orch.DEFAULT_CONFIG["review_profiles"], "QUANT": ["OPUS_FINAL"],
    }}
    with pytest.raises(ValueError, match="NON_MONOTONIC_REVIEW_PROFILE"):
...
def test_review_enqueue_never_creates_pre_review_merge_gate(tmp_path):
    ...
    class GitHubStub:
        ...
        def changed_files(self, number):
            return ["scripts/ai_team_orchestrator.py"]
