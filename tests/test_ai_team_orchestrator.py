...
def test_explicit_review_profile_may_upgrade_and_invalid_fails_closed():
...
@pytest.mark.parametrize(
    ("task_class", "requested"),
    [
        ("QUANT_PROFITABILITY", "ROUTINE"),
        ("QUANT_PROFITABILITY", "ENGINE_CRITICAL"),
        ("STATISTICAL_METHODOLOGY", "ROUTINE"),
        ("STATISTICAL_METHODOLOGY", "ENGINE_CRITICAL"),
        ("CAPITAL_SENSITIVE_METHODOLOGY", "ROUTINE"),
        ("CAPITAL_SENSITIVE_METHODOLOGY", "ENGINE_CRITICAL"),
        ("MAJOR_ARCHITECTURE", "ROUTINE"),
        ("DESTRUCTIVE", "ROUTINE"),
    ],
)
def test_high_risk_task_class_cannot_downgrade_review_profile(task_class, requested):
    with pytest.raises(ValueError, match="REVIEW_PROFILE_BELOW_REQUIRED_FLOOR"):
        orch.parse_review_profile(
            f"AI_TASK_CLASS={task_class}\nAI_TEAM_REVIEW_PROFILE={requested}",
            orch.DEFAULT_CONFIG,
            task_class,
        )
...
@pytest.mark.parametrize(
    "sensitivity", ["AI_TEAM_PROTECTED_CHANGE=YES", "LIVE-SENSITIVE: YES"]
)
def test_sensitive_control_plane_cannot_downgrade_review_profile(sensitivity):
    with pytest.raises(ValueError, match="required=ENGINE_CRITICAL"):
        orch.parse_review_profile(
            f"AI_TASK_CLASS=ROUTINE\n{sensitivity}\nAI_TEAM_REVIEW_PROFILE=ROUTINE",
            orch.DEFAULT_CONFIG,
            "ROUTINE",
        )


@pytest.mark.parametrize("upgrade", ["ENGINE_CRITICAL", "QUANT", "DESTRUCTIVE"])
def test_routine_task_may_explicitly_upgrade_review_profile(upgrade):
    body = f"AI_TASK_CLASS=ROUTINE\nAI_TEAM_REVIEW_PROFILE={upgrade}"
    assert orch.parse_review_profile(body, orch.DEFAULT_CONFIG, "ROUTINE") == upgrade


def test_persisted_task_profile_is_revalidated_fail_closed(tmp_path):
    ledger = orch.Ledger(tmp_path / "ledger.sqlite3")
    task_id = ledger.create_task(
        issue_number=223, task_type="REVIEW", agent="CODEX_REVIEWER",
        model_class="CODEX_DEFAULT", task_class="QUANT_PROFITABILITY",
        review_profile="ROUTINE",
    )
    with pytest.raises(RuntimeError, match="REVIEW_PROFILE_BELOW_REQUIRED_FLOOR"):
