...
def test_durable_wrapper_never_exposes_legacy_publication(tmp_path: Path, monkeypatch) -> None:
    ...
    public_path.write_text(json.dumps({"contract": "durable-last-known-good"}), encoding="utf-8")
    weak_payload = {
        "version": 1,
        "source": "invo",
        "verified_count": 1,
        "identities": [{"resolver_rule_version": "sqd-public-trade-v3-size-aware-sequence"}],
    }

    async def fake_run_once(args: Namespace) -> dict[str, object]:
        legacy_identifier_job._save_object(args.state_dir / "identified_wallets.json", weak_payload)
        # The public durable surface must remain untouched while the legacy resolver runs.
        assert json.loads(public_path.read_text(encoding="utf-8")) == {
            "contract": "durable-last-known-good"
        }
        return {"attempted": 1}

    monkeypatch.setattr(legacy_identifier_job, "run_once", fake_run_once)
    result = asyncio.run(
        _run_legacy_resolver_without_publication(
            Namespace(state_dir=tmp_path)
        )
    )
    ...
    private = json.loads(
        (tmp_path / "identified_wallets_legacy_diagnostic.json").read_text(encoding="utf-8")
    )
...
def test_verified_shadow_sync_uses_same_pipeline_flock_as_identifier() -> None:
    ...
    identifier = (root / "deploy/systemd/hyperliquid-invo-wallet-identifier.service").read_text()
    sync = (root / "deploy/systemd/hyperliquid-invo-verified-shadow-sync.service").read_text()
