...
stable first-ready queue delay; portfolio/candidate/verified p50/p90/p99 latency; proof-stage
reduction totals; verified, unresolved and error yield; and SQD request count, cumulative
request latency and retry count both batch-wide and per portfolio. Tier B remains bounded to
eight discovery windows plus at most six candidates times twelve held-out windows per trader.
These measurements are emitted even at zero yield; unresolved traders remain in the resolution
queue for Lane 3 rather than being promoted as identities.
