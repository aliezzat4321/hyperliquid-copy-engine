...
## 2026-09-13 — Review profiles have monotonic risk floors

`AI_TEAM_REVIEW_PROFILE` is an upgrade request, not an override. The orchestrator derives
the minimum profile from the declared task class and protected/live-sensitive metadata,
then fails closed if the requested profile is weaker. Routine work may still request any
stronger profile; quant/statistical/capital-sensitive work retains `QUANT`, major
architecture and destructive work retain `DESTRUCTIVE`, and sensitive routine
control-plane work retains at least `ENGINE_CRITICAL`.
