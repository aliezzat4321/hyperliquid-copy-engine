...
| Elite direct watch | `services/invo-notification-executor/src/elite-direct-watch.ts`, `invo-client.ts` | Shadow-only dedicated read-only `/v1_0/investments/get_investments` loop; two-phase OPEN+CLOSED baseline before admission, no arbitrary selected-trader count exclusion, fair bounded hydration scheduling, timeout/page/deadline transport proof, bounded tombstones/deferred state, append journal, compact admission index, and process watchdog recovery |
