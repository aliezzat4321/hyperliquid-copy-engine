...
| AI-team runner recovery | `scripts/ai_team_runner_recovery.py`, `scripts/install_ai_team_supervisor.sh`, `deploy/systemd/hyperliquid-ai-team-supervisor.service` | Bounded, cooldown-protected supervisor `ExecStartPre`; may restart only the exact configured, already-enabled production Actions runner and does not register runners or mutate credentials or trading state |
...
| Prospective elite admission | `services/invo-notification-executor/src/elite-admission.ts` | NEW/ADD requires exact-portfolio pre-trade elite evidence; the feed is the primary admission path (`requireDirectWatchAdmission=false`, keyed off wakeSource, admits on that causal evidence alone) and the compact direct-watch admission index gates only direct-watch's own `elite_direct:`-sourced signals (reconciliation/fallback); CLOSE bypasses admission for managed unwind |
| Elite direct watch | `services/invo-notification-executor/src/elite-direct-watch.ts`, `invo-client.ts` | Shadow-only dedicated read-only `/v1_0/investments/get_investments` loop paced by the coordinated Invo request budget at the lower priority class; two-phase OPEN+CLOSED baseline before admission, no arbitrary selected-trader count exclusion, fair bounded hydration scheduling, timeout/page/deadline transport proof, bounded tombstones/deferred state, append journal, compact admission index, and process watchdog recovery |
...
| Invo API client | `services/invo-notification-executor/src/invo-client.ts` | `/v1_0/posts/get_feed`; parses `Retry-After` onto `InvoHttpError`; **live-sensitive** (`/dex/position/*`) |
| Coordinated Invo request budget | `services/invo-notification-executor/src/invo-request-budget.ts` | One account-wide token bucket for every Invo request in the process. Two priority classes (`FEED` primary, `DIRECT_WATCH` reconciliation): direct watch may not consume the reserved feed tokens and never overtakes a queued feed request, FIFO within a class, adaptive `Retry-After`/exponential 429 cooldown with AIMD rate reduction, bounded fail-closed waits |
