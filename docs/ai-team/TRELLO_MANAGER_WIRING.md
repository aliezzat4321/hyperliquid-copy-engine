...
6. Treat exit status and `TRELLO_SYNC=DEFERRED` as observability only. Log only the issue/event and deferred reason class. Never fail, delay, or roll back the engineering transition because Trello failed. Every reconciliation rechecks visible cards on board `6a9713c265a75ed50d4181d7` using the title's exact issue token, canonical issue URL, or canonical `Issue: #<issue>` field; PR/SHA references are not identity. Prefer the deterministic pre-existing project card over an exact synthetic `[P?] #N AI team task`, update the canonical card successfully, then archive only positively identified synthetic duplicates. Create a card only when no exact match exists, and preserve an established project title when a runtime event has no title.
7. Worker and task completion are nonterminal projections. Build/repair running or finished projects to In Progress; Claude review and CI projects to Review / CI; review failure followed by repair returns to In Progress; genuine blockers project to Blocked / Needs Decision. Only the explicit completion event emitted after durable GitHub issue closure plus `ai-team:done`, or its saved terminal marker, may project Done / Proven. Ledger reconciliation uses the newest task to repair active state and never infers issue completion from a worker `DONE` status alone. Reconciliation-generated repairs do not emit Trello comments, so repeated bounded passes remain idempotent and notification-free.
