...
As part of the same trusted manager patch, update `docs/ai-team/AUTONOMOUS_TEAM.md` to describe `ai-team:queued` plus the explicit metadata/dependency contract above as the autonomous entry point, while retaining `ai-team:ready` as the direct manual entry point. That documentation file is outside the autonomous builder allowlist, so this checkout intentionally does not edit it.
