...
from hlcopy.shadow.wide_enrich import JsonlOfficialFillSink
from hlcopy.shadow.wide_enrich_live import ProspectiveWideTradeOfficialEnricher


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="python -m hlcopy.shadow.wide_enrich_cli_live")
    parser.add_argument("--source-dir", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--checkpoint", type=Path, required=True)
    parser.add_argument("--poll-seconds", type=float, default=0.5)
    parser.add_argument("--query-window-ms", type=int, default=2_000)
    parser.add_argument("--max-event-age-ms", type=float, default=10_000.0)
    return parser
