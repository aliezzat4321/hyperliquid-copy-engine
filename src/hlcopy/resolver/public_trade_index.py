...
class PublicTradeDiscoveryConfig:
    ...
    max_parallel_anchor_queries: int = 4
    max_parallel_verification_queries: int = 2
...
async def discover_candidates(
    ...
    *,
    ...
    async def match_anchor(signal: CopySignal) -> tuple[str, dict[str, AnchorMatch]]:
...
async def verify_candidate_historically(
    *,
    ...
    lookback_cutoff_ms: int | None = None
    if covered:
        newest_close_ms = max(signal.closed_at_ms for signal in covered)
    ...
    eligible = [
        ...
        and (lookback_cutoff_ms is None or signal.closed_at_ms >= lookback_cutoff_ms)
    ]
    ...
    async def match_signal(signal: CopySignal) -> EpisodeEvidence:
        async with semaphore:
    ...
    matched_evidence = await asyncio.gather(*(match_signal(signal) for signal in selected))
    evidence: list[EpisodeEvidence] = []
    matched_ids: list[str] = []
    used_lifecycle_ids: set[str] = set()
    for signal, resolved_item in zip(selected, matched_evidence, strict=True):
        item = resolved_item
        if item.matched:
...
async def resolve_source_public_trades(
    *,
    ...
    report = {
        "version": 10,
        "resolver_rule_version": "sqd-fill-wallet-identity-v10",
