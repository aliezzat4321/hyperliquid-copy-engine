...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--max-parallel-anchor-queries", type=int, default=4)
    parser.add_argument("--max-parallel-verification-queries", type=int, default=2)
...
async def _run(args: argparse.Namespace) -> None:
    config = PublicTradeDiscoveryConfig(
        ...
        max_parallel_anchor_queries=max(1, args.max_parallel_anchor_queries),
        max_parallel_verification_queries=max(
            1, args.max_parallel_verification_queries
        ),
