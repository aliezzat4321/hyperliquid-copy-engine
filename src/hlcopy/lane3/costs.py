...
def measure_leg(
    ...
    *,
    ...
    book = provider.first_at_or_after(position.coin, leg.timestamp_ms)
    fee = leg.notional * taker_rate
    # Never mislabel a stale quote as decision-time executable evidence.
    if book is None or book.received_at_ns < leg.timestamp_ms * 1_000_000:
        return LegCost(leg, fee, None, None, None, None, CostCompleteness.UNMEASURED_NO_BOOK)
    ...
    if not fill.complete or fill.slippage_bps is None or not levels:
        return LegCost(leg, fee, None, None, None, fill.slippage_bps,
                       CostCompleteness.CAPACITY_INFEASIBLE)
    best = levels[0].price
    half_bps = ((best - leg.mid) if buy else (leg.mid - best)) / leg.mid * BPS
    impact_bps = fill.slippage_bps - half_bps
    return LegCost(leg, fee, leg.notional * fill.slippage_bps / BPS,
                   leg.notional * half_bps / BPS, leg.notional * impact_bps / BPS,
                   fill.slippage_bps, CostCompleteness.MEASURED)
