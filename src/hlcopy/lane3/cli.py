...
def coverage(args: argparse.Namespace) -> dict[str, Any]:
    ...
    provider = Lane3ForwardBookProvider(
    ...
    for coin, timestamp in decisions:
        book = provider.first_at_or_after(coin, timestamp)
        if book is not None and book.received_at_ns >= timestamp * 1_000_000:
            measured += 1
...
def _slice(
    ...
    for position in closed:
        ...
        costs = [measure_leg(position, leg, provider, taker_rate=D(str(cfg["taker_rate"])),
                             max_slippage_bps=D(str(cfg["max_slippage_bps"]))) for leg in legs]
